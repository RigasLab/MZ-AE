...
class Eval_MZA_Experiment(MZA_Experiment):
    ...
    def predict_onestep(self, dataset, num_trajs):
        ...
        Phi_nn_hat     = torch.movedim(re_Phi_nn_hat, 1, 0) #[num_trajs timesteps obsdim]

        re_Phi_nn    = Phi_nn.reshape(int(Phi_nn.shape[0]/num_trajs), num_trajs, *Phi_nn.shape[1:])
        Phi_nn       = torch.movedim(re_Phi_nn, 1, 0) #[num_trajs timesteps obsdim]


        return x_nn_hat.detach(), Phi_nn_hat.detach(), Phi_nn.detach()#avg_loss, avg_ObsEvo_Loss, avg_Autoencoder_Loss, avg_StateEvo_Loss, avg_koop_ptg, avg_seqmodel_ptg
