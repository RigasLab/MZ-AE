...
class Train_Methodology():
    ...
    def time_evolution(self, initial_x_n, initial_x_seq, initial_Phi_n, ph_size):
        ...
        for t in range(ph_size):
            ...
            koop_out = self.DDPmodel(x_n, module = "koopman", function = "forward")
            ...
            if self.deactivate_seqmodel:
             ...
             #collecting seqmodel prediction
                ...
                seqmodel_out = self.DDPmodel(x_seq, module = "seqmodel", function = "forward")
                if t == 0:                      
            ...
            Phi_nn_hat = self.DDPmodel(x_nn_hat, module = "autoencoder", function = "recover")
    ...
    def train_test_loss(self, mode = "Train", dataloader = None):
        ...
        for Phi_seq, Phi_nn_ph in dataloader:
            ...
            ###### obtain observables ######
            x_seq, Phi_seq_hat = self.DDPmodel(Phi_seq, module = "autoencoder", function = "forward")
            x_nn_ph , Phi_nn_hat_ph_nolatentevol = self.DDPmodel(Phi_nn_ph, module = "autoencoder", function = "forward")
    ...
    def training_loop(self):
        ...
        print(f"Test Loss: {test_Ldict['avg_loss']:<{6}}, KoopEvo : {test_Ldict['avg_KoopEvo_Loss']:<{6}}, Residual : {test_Ldict['avg_Residual_Loss']:<{6}}, Auto : {test_Ldict['avg_Autoencoder_Loss']:<{6}}, StateEvo : {test_Ldict['avg_StateEvo_Loss']:<{6}}, LatentEvo : {test_Ldict['avg_LatentEvo_Loss']}", flush = True)
        ...
        for ix_epoch in range(self.load_epoch, self.load_epoch + self.nepochs):
            ...
            if not self.deactivate_lrscheduler:
                ...
                if self.gpu_id == 0:
                    print("Epoch %d: SGD lr %.6f -> %.6f" % (ix_epoch, before_lr, after_lr), flush = True)
            ...
            if self.nepoch_actseqmodel!=0:
                if ix_epoch == self.nepoch_actseqmodel:
                    ...
                    if self.gpu_id == 0:
                        print("SEQMODEL : ", not self.deactivate_seqmodel, flush = True)
            ...
            if self.gpu_id == 0:
                print(f"Epoch {ix_epoch} ", flush = True)
                print(f"Train Loss: {train_Ldict['avg_loss']:<{6}}, KoopEvo : {train_Ldict['avg_KoopEvo_Loss']:<{6}}, Residual : {train_Ldict['avg_Residual_Loss']:<{6}}, Auto : {train_Ldict['avg_Autoencoder_Loss']:<{6}}, StateEvo : {train_Ldict['avg_StateEvo_Loss']:<{6}}, LatentEvo : {train_Ldict['avg_LatentEvo_Loss']}", flush = True)
                print(f"Test Loss: {test_Ldict['avg_loss']:<{6}}, KoopEvo : {test_Ldict['avg_KoopEvo_Loss']:<{6}}, Residual : {test_Ldict['avg_Residual_Loss']:<{6}}, Auto : {test_Ldict['avg_Autoencoder_Loss']:<{6}}, StateEvo : {test_Ldict['avg_StateEvo_Loss']:<{6}}, LatentEvo : {test_Ldict['avg_LatentEvo_Loss']}", flush = True)
