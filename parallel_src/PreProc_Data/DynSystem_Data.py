...
from parallel_src.PreProc_Data.DataProc import StackedSequenceDataset
...
class DynSystem_Data:
    ...
    def load_and_preproc_data(self, mode = "Both"):
        '''
        ...
        Dataset (both test and train) : [num_traj, timesteps, statedim] Input , Output (both test and train)
        '''
    ...
    def create_dataset(self, mode = "Both"):
        ...
        '''
        if mode == "Both" or mode == "Train":
            self.train_dataloader = DataLoader(self.train_dataset, batch_size=self.batch_size, shuffle = False, sampler = DistributedSampler(self.train_dataset))
        
        # print("out of train")
        if mode == "Both" or mode == "Test":
            self.test_dataloader  = DataLoader(self.test_dataset, batch_size=self.batch_size, shuffle = False, sampler = DistributedSampler(self.test_dataset))
