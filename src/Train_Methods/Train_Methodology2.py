...
class Train_Methodology2():
    ...
    def koopman_train_loss(self, dataloader = None):
        '''
        One Step Prediction method with RNN focusing on residual and taking one step back in the sequence data(i.e. not ending at xn but at xn-1)
        Requires
        --------
        ...
        self.model.train()
        self.model.autoencoder.weights.requires_grad = True
        self.model.koopman.weights.requires_grad     = True
        self.model.seqmodel.weights.requires_grad    = False
        # elif mode == "Test":
        ...
        for Phi_seq, Phi_n, Phi_nn, indices in dataloader:
            ...
            #calculating residuals for RNN
            residuals = x_nn - x_nn_hat
            dataset.residuals[indices] = residuals 
    ...
    def seqmodel_train_loss(self, dataloader = None):
        '''
        Residual prediction by sequence model
        Requires
        --------
        Dataloader, model, optimizer
        '''
        ...
        self.model.autoencoder.weights.requires_grad = False
        self.model.koopman.weights.requires_grad     = False
        self.model.seqmodel.weights.requires_grad    = True
        ...
        for Phi_seq, Phi_n, Phi_nn, indices in dataloader:
            ...
            #obtain observables
            x_seq, _ = self.model.autoencoder(Phi_seq)
            ...
            #Actual Residuals
            residuals = dataset.residuals[indices]

            #Calculating loss
            mseLoss   = nn.MSELoss()
            seq_loss  = mseLoss(residuals, seqmodel_out)
    ...
    def test_loss(self, dataloader = None):
        '''
    ...
    def training_loop(self):
        ...
        test_loss, test_ObsEvo_Loss, test_Autoencoder_Loss, test_StateEvo_Loss, test_koop_ptg, test_seqmodel_ptg = self.test_loss("Test", self.test_dataloader)
...
for Phi_seq, Phi_n, Phi_nn in dataloader:
